...
app.put("/template/:id", async (req, res) => {
    ...
    if (!req.params.id || !templates[req.params.id]) {
        return res.sendStatus(400);
    }
    const manager = templates[req.params.id];

    // Process all the updates sent from the frontend
    for (const key in req.body) {
        if (key === "running") {
            // This handles starting and stopping the template
            ...
            // This is the new logic to properly handle user list changes
            const newUserIds = req.body.userIds;
            if (!newUserIds || newUserIds.length === 0) {
                return res.status(400).json({ error: "A template must have at least one user." });
            }
            
            // Update the user list and all related properties
            manager.userIds = newUserIds;
            manager.masterId = manager.userIds[0];
            manager.masterName = users[manager.masterId].name;
            manager.masterIdentifier = manager.userIds.map(id => `${users[id].name}#${id}`).join(', ');

        } else {
            // For any other setting (like name, coords, etc.), just update it
            manager[key] = req.body[key];
        }
    }

    // This is the critical step: save all changes to the file
    saveTemplates();
