...
// <<< START OF ADDED CODE: Elements for the new Edit tab >>>
const editTemplate = $("editTemplate");
const editTemplateForm = $("editTemplateForm");
const editTemplateId = $("editTemplateId");
const editTemplateName = $("editTemplateName");
const editTx = $("editTx");
const editTy = $("editTy");
const editPx = $("editPx");
const editPy = $("editPy");
const editUserSelectList = $("editUserSelectList");
const editSelectAllUsers = $("editSelectAllUsers");
const editCanBuyMaxCharges = $("editCanBuyMaxCharges");
const editCanBuyCharges = $("editCanBuyCharges");
const editAntiGriefMode = $("editAntiGriefMode");
// <<< END OF ADDED CODE >>>
...
// users
let usersData = {}; // <<< ADDED: Global variable to store user data
const loadUsers = async (f) => {
    try {
        const users = await axios.get("/users");
        usersData = users.data; // <<< ADDED: Store user data globally
        if (f) f(users.data);
    } catch (error) {
        handleError(error);
    };
...
let templatesData = {}; // <<< ADDED: Global variable for template data
const loadTemplates = async (f) => {
    try {
        const templates = await axios.get("/templates");
        templatesData = templates.data; // <<< ADDED: Store template data globally
...
let currentTab = main;
const changeTab = (el) => {
    // <<< MODIFIED: Hide all tabs, including the new 'editTemplate' tab >>>
    ["main", "manageUsers", "addTemplate", "manageTemplates", "settings", "editTemplate"].forEach(tabId => {
        const tab = $(tabId);
...
// <<< START OF ADDED CODE: Function to open the new Edit Tab >>>
const openEditTemplateTab = (id) => {
    const template = templatesData[id];
    if (!template) {
        handleError({ message: "Could not find template data to edit." });
        return;
    }
    ...
    [editTx.value, editTy.value, editPx.value, editPy.value] = template.coords;
    
    // Populate users with correct ones checked
    editUserSelectList.innerHTML = "";
    Object.keys(usersData).forEach(userId => {
        const user = usersData[userId];
        const isChecked = template.userIds.includes(userId);
        const userDiv = document.createElement('div');
        userDiv.className = 'user-select-item';
        userDiv.innerHTML = `
            <input type="checkbox" id="edit_user_${userId}" name="edit_user_checkbox" value="${userId}" ${isChecked ? 'checked' : ''}>
            <label for="edit_user_${userId}">${user.name} (#${userId})</label>
        `;
        editUserSelectList.appendChild(userDiv);
    });

    editCanBuyMaxCharges.checked = template.canBuyMaxCharges;
    editCanBuyCharges.checked = template.canBuyCharges;
    editAntiGriefMode.checked = template.antiGriefMode;
    
    changeTab(editTemplate);
};
// <<< END OF ADDED CODE >>>
    ...
    loadUsers(users => {
        loadTemplates(templates => {
            for (const id of Object.keys(templates)) {
                ...
                // <<< MODIFIED: Added Edit button with an onclick event >>>
                ...
                // <<< START OF ADDED CODE: Edit button >>>
                const editButton = document.createElement('button');
                editButton.className = 'secondary-button';
                editButton.innerHTML = '<img src="icons/settings.svg">Edit Template';
                editButton.addEventListener('click', () => openEditTemplateTab(id));
                // <<< END OF ADDED CODE >>>
                ...
                buttons.append(editButton); // <<< ADDED
...
// <<< START OF ADDED CODE: Event listener for the new Edit Form >>>
editTemplateForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const id = editTemplateId.value;
    const selectedUsers = Array.from(document.querySelectorAll('input[name="edit_user_checkbox"]:checked')).map(cb => cb.value);
    ...
    try {
        await axios.put(`/template/${id}`, {
            ...
            coords: [editTx.value, editTy.value, editPx.value, editPy.value].map(Number),
            userIds: selectedUsers,
            canBuyCharges: editCanBuyCharges.checked,
            canBuyMaxCharges: editCanBuyMaxCharges.checked,
            antiGriefMode: editAntiGriefMode.checked,
        });
        showMessage("Success", "Template updated successfully!");
        openManageTemplates.click(); // Go back to the manage templates screen
    } catch(error) {
        handleError(error);
    }
});

editSelectAllUsers.addEventListener('click', () => {
    document.querySelectorAll('#editUserSelectList input[type="checkbox"]').forEach(cb => cb.checked = true);
});
// <<< END OF ADDED CODE >>>